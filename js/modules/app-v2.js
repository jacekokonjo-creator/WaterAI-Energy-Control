...
const ANAL_STYLE = `<style>
  ...
  @media print{
    ...
    .anw-formula,.anw-pair,.anw-pair-col,table.anw-t,table.anw-bvs,table.anw-src,.anw-unit-idx,.anw-g2,.anw-rgrid,.anw-tile,.anw-sign,.anw-chart-wrap canvas{break-inside:avoid;page-break-inside:avoid;}
...
function _analLongText(v) {
  ...
  // Treść analityka tłumaczy NoteI18n (maszynowo, przez Edge Function), a nie silnik
  // słownikowy — ten podmieniałby w prozie pojedyncze słowa („Gaz", „Opis", „Hotel")
  // i dawał mieszankę językową. Fallback: gdy moduł nie wstał, zostaje oryginał.
  if (!t) return '';
  if (window.NoteI18n) return NoteI18n.wrap(t, { inline: true });
  return `<div data-i18n-skip style="white-space:pre-wrap;line-height:1.55;">${_escA(t)}</div>`;
}
...
const IntensityBaseModule = {
  ...
  add(it) { const a = this.getAll(); a.push({ ...it, id: (window._waNextIdFor ? _waNextIdFor(a) : Date.now()), createdAt: new Date().toISOString() }); this.saveAll(a); },
...
const BasePeriodModule = {
  ...
  add(it) { const a = this.getAll(); const rec = { ...it, id: (window._waNextIdFor ? _waNextIdFor(a) : Date.now()), createdAt: new Date().toISOString() }; a.push(rec); this.saveAll(a); return rec; },
  update(id, it) { this.saveAll(this.getAll().map(x => Number(x.id) === Number(id) ? { ...x, ...it, id: x.id, updatedAt: new Date().toISOString() } : x)); },
  remove(id) { this.saveAll(this.getAll().filter(x => Number(x.id) !== Number(id))); },
  find(id) { return this.getAll().find(x => Number(x.id) === Number(id)); },
