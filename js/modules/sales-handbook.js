...
const SECTIONS=[["start","Start"],["klienci","Klient"],["spotkanie","Spotkanie"],["wiedza","Wiedza"],["trening","Trening"]];
const SEG_TABS=[["persona","Kto decyduje"],["diag","Otwarcie i diagnoza"],["args","Argumenty"],["obj","Obiekcje"],["path","Oferta i ścieżka"],["dialog","Dialog"]];
const SPOT_GROUPS=[["Przed spotkaniem",[["brief","Jutro mam spotkanie","briefing z segmentu"],["before","Checklista: przed",""],["q20","20 pytań","które zmuszają do liczenia"],["site","Wizja techniczna","zanim obiecasz technicznemu"]]],["W trakcie",[["m30","Spotkanie 30 min",""],["m60","Spotkanie 60 min",""],["during","Checklista: w trakcie",""],["close20","20 zdań do decyzji",""],["avoid20","20 zdań do unikania",""]]],["Po spotkaniu",[["after","Checklista: po","do 24 godzin"],["fu","Schemat follow-upu",""],["maile","Generator maili","gotowy mail do skopiowania"]]],["Cały proces",[["process","Proces sprzedaży","etapy i warunki przejścia"],["stages","Strategia etapów","1. spotkanie → pilotaż → zamknięcie"]]]];
const TOOL_ITEMS=[].concat(...SPOT_GROUPS.map(g=>g[1])).filter(x=>x[0]!=="maile"&&x[0]!=="brief").map(([id,l])=>[id,l]);
const KNOW_ITEMS=[["prawo","Prawo i regulacje","status: obowiązek / projekt / przyszłe"],["obiekcje","Biblioteka obiekcji","krótko, rozwinięcie, riposta"],["calc","Jak liczymy oszczędność","baseline, normalizacja, korekty"],["esco","Umowa ESCO","kiedy ma sens, co w umowie"]];
const ACAD_ITEMS=[["trening","Trening obiekcji","losowa obiekcja, oceń się"],["sym","Symulator klienta","napisz odpowiedź, sprawdź"],["fiszki","Fiszki","30 pytań i odpowiedzi"],["egzamin","Egzamin","20 pytań, 80 % zalicza"]];
const DECIDER_MAP={urzad:["Burmistrz / wójt / prezydent","Skarbnik"],oswiata:["Burmistrz / wójt / prezydent","Skarbnik"],szpital:["CFO","Dyrektor techniczny / chief engineer"],hotel:["Dyrektor hotelu (GM)","Dyrektor techniczny / chief engineer"],spoldzielnia:["Prezes spółdzielni"],wspolnota:["Zarządca nieruchomości"],biurowiec:["Asset manager / fundusz","Zarządca nieruchomości","Dyrektor techniczny / chief engineer"],galeria:["Asset manager / fundusz","Dyrektor techniczny / chief engineer"],przemysl:["Główny energetyk","CFO"],siec:["CFO","Dyrektor techniczny / chief engineer"],pec:["Prezes przedsiębiorstwa ciepłowniczego"],operator:["Prezes przedsiębiorstwa ciepłowniczego"],esco:["CFO"],fm:["Zarządca nieruchomości","Dyrektor techniczny / chief engineer"],inne:[]};
cur=cur||{sec:"start",seg:"urzad",tab:"persona",tool:"brief",objcat:"Wszystkie",hitsMode:false,acad:"trening",calc:"prawo"};
MAIL=MAIL||{want:null};
...
function plist(items,key,curVal){return `<ul class="plist">`+items.map(([id,l,sm])=>`<li><button data-${key}="${id}" class="${curVal===id?"on":""}">${l}${sm?`<small>${sm}</small>`:""}</button></li>`).join("")+`</ul>`;}
function renderSide(){
  const side=$id("side");let html="";
  if(cur.sec==="start"){html=`<p class="grp">Po co tu jesteś?</p><ul class="plist"><li><button data-go='{"sec":"spotkanie","tool":"brief"}'>Mam jutro spotkanie<small>briefing z segmentu</small></button></li><li><button data-go='{"sec":"wiedza","calc":"obiekcje","objcat":"Wszystkie"}'>Klient powiedział…<small>biblioteka obiekcji</small></button></li><li><button data-go='{"sec":"trening","acad":"trening"}'>Chcę poćwiczyć<small>trening, symulator, egzamin</small></button></li></ul><p class="grp">W tym rozdziale</p><ul class="plist"><li><button disabled>Co sprzedajesz</button></li><li><button disabled>Sekwencja rozmowy</button></li><li><button disabled>Pitch 30 s</button></li><li><button disabled>Liczby i referencje</button></li><li><button disabled>Czego nie mówić</button></li></ul>`;}
  else if(cur.sec==="klienci"){const groups=[...new Set(SEGMENTS.map(s=>s.group))];groups.forEach(g=>{html+=`<p class="grp">${g}</p>`+plist(SEGMENTS.filter(s=>s.group===g).map(s=>[s.id,s.name,s.short]),"seg",cur.seg);});}
  else if(cur.sec==="spotkanie"){SPOT_GROUPS.forEach(([g,items])=>{html+=`<p class="grp">${g}</p>`+plist(items,"tool",cur.tool);});}
  else if(cur.sec==="wiedza"){html=`<p class="grp">Wiedza</p>`+plist(KNOW_ITEMS,"calc",cur.calc);
    if(cur.calc==="obiekcje"){const cats=["Wszystkie",...new Set(OBJ_LIB.map(o=>o.c)),"Riposty"];html+=`<p class="grp">Kategorie</p><ul class="plist">`+cats.map(c=>`<li><button data-cat="${c}" class="${cur.objcat===c?"on":""}">${c}<small>${c==="Wszystkie"?OBJ_LIB.length+" obiekcji":c==="Riposty"?RIPOSTES.length+" zdań":OBJ_LIB.filter(o=>o.c===c).length+" obiekcji"}</small></button></li>`).join("")+`</ul>`;}
    else if(cur.calc==="prawo"){html+=`<p class="grp">Legenda</p><div style="font-size:13px;line-height:2">${badge("OBOWIĄZEK")}<br>${badge("PROJEKT / W TRAKCIE WDRAŻANIA")}<br>${badge("PRZYSZŁE / PO TRANSPOZYCJI")}<br>${badge("KIERUNEK / ZALEŻNE")}</div>`;}}
  else if(cur.sec==="trening"){html=`<p class="grp">Ćwiczenia</p>`+plist(ACAD_ITEMS,"acad",cur.acad)+`<p class="grp">Materiał</p><ul class="plist"><li><button data-go='{"sec":"wiedza","calc":"obiekcje","objcat":"Wszystkie"}'>→ Biblioteka obiekcji</button></li><li><button data-go='{"sec":"wiedza","calc":"prawo"}'>→ Prawo</button></li></ul>`;}
  side.innerHTML=html;
}

function pecChapter(){return `<div class="paper"><h3>Punkt wyjścia</h3><p>${hl(PEC_CHAPTER.intro)}</p><h3>Konflikt interesów – nazwij go, zanim zrobi to klient</h3>${ul(PEC_CHAPTER.conflict)}</div><h3 style="font-size:14px;color:var(--ink-2);margin:0 0 8px">Argumenty i ich weryfikacja techniczna</h3>`+PEC_CHAPTER.args.map(a=>`<details><summary>${hl(a.t)}${badge(a.v)}</summary><div class="body"><p class="lab">Dlaczego / kiedy</p><p>${hl(a.why)}</p><p class="lab">Jak udowodnić</p><p>${hl(a.how)}</p></div></details>`).join("")+`<div class="paper" style="margin-top:16px"><h3>Z kim o czym w PEC</h3><table><tr><th>Rola</th><th>Jak rozmawiać</th></tr>${PEC_CHAPTER.roles.map(([r,t])=>`<tr><td><b>${esc(r)}</b></td><td>${hl(t)}</td></tr>`).join("")}</table></div>`;}
function deciderTypes(){return `<div class="paper"><p>${hl(DECIDERS.intro)}</p></div><div class="cols">`+DECIDERS.types.map(t=>`<div class="paper"><h3>${esc(t.t)}</h3><p class="lab" style="font-family:'IBM Plex Sans';font-size:13px;color:var(--ink-2)">Czego chce</p><p>${hl(t.wants)}</p><p class="lab" style="font-family:'IBM Plex Sans';font-size:13px;color:var(--ink-2)">Rób</p><p>${hl(t.do)}</p><p class="lab" style="font-family:'IBM Plex Sans';font-size:13px;color:var(--ink-2)">Nie rób</p><p>${hl(t.dont)}</p></div>`).join("")+`</div>`;}
function renderView(){
  const view=$id("view"),tabs=$id("tabs");tabs.innerHTML="";
  if(cur.hitsMode){renderHits();return;}
  if(cur.sec==="start"){setTitle("Start","To, co musi być w głowie przed każdą rozmową.");view.innerHTML=`<div class="bar" style="margin:0 0 14px"><button data-go='{"sec":"spotkanie","tool":"brief"}'>Mam jutro spotkanie</button><button class="ghost" data-go='{"sec":"wiedza","calc":"obiekcje","objcat":"Wszystkie"}'>Klient powiedział…</button><button class="ghost" data-go='{"sec":"trening","acad":"trening"}'>Chcę poćwiczyć</button></div>`+`<div class="paper"><h3>Co naprawdę sprzedajesz</h3><p>${hl(SEQUENCE.principle)}</p><h3>Sekwencja każdej rozmowy</h3><p class="seq">${SEQUENCE.steps.map(x=>`<span>${esc(x)}</span>`).join("<i>→</i>")}</p><h3>Otwarcie, które działa wszędzie</h3><p>${hl(SEQUENCE.opening)}</p><h3>Pitch w 30 sekund</h3><p>${hl(CORE.pitch)}</p><h3>Prośba o spotkanie</h3><p>${hl(CORE.askMeeting)}</p><h3>Jak liczymy oszczędność</h3><ol>${CORE.method.map(x=>`<li>${hl(x)}</li>`).join("")}</ol><h3>Co sprzedajesz – dwa elementy</h3>${ul(CORE.product)}<h3>Jak mówić o liczbach</h3>${ul(CORE.numbers)}<h3>Referencje (waterai.pl)</h3><table class="tbl"><tr><th>Obiekt</th><th>Segment</th><th>Wynik</th></tr>${CORE.refs.map(r=>`<tr><td>${hl(r.name)}</td><td>${hl(r.seg)}</td><td>${hl(r.result)}</td></tr>`).join("")}</table><p class="small">${hl(CORE.refsNote)}</p><h3>Czego nie mówić</h3>${ul(CORE.dontSay)}<h3>Dane, które zbierasz do oferty</h3>${checklist(CORE.data,"core")}</div>`;return;}
  if(cur.sec==="wiedza"&&cur.calc==="prawo"){setTitle("Prawo i regulacje – stan na wrzesień 2026","Każdy przepis: artykuł, kogo dotyczy, czego wymaga, jak użyć zgodnie z prawdą, pułapka.");view.innerHTML=`<div class="paper"><p style="font-family:'IBM Plex Sans',sans-serif;font-size:13.5px;color:var(--ink-2)">${hl(LEGAL_NOTE)}</p></div>`+LEGAL.map(l=>`<details><summary>${hl(l.act)} – ${hl(l.art)}${badge(l.status)}</summary><div class="body"><p class="lab">Kogo dotyczy</p><p>${hl(l.who)}</p><p class="lab">Czego wymaga</p><p>${hl(l.what)}</p><p class="lab">Jak użyć w rozmowie</p><p>${hl(l.use)}</p><p class="lab">Pułapka – czego nie mówić</p><p>${hl(l.trap)}</p></div></details>`).join("");return;}
  if(cur.sec==="wiedza"&&cur.calc==="obiekcje"){setTitle("Biblioteka obiekcji","Krótka odpowiedź na spotkanie, rozwinięcie do dalszej rozmowy. "+OBJ_LIB.length+" obiekcji z kategoriami + "+RIPOSTES.length+" ripost jednym zdaniem.");const v=q();let list=cur.objcat==="Riposty"?[]:OBJ_LIB.filter(o=>cur.objcat==="Wszystkie"||o.c===cur.objcat);if(v)list=list.filter(o=>(o.q+o.s+o.l).toLowerCase().includes(v.toLowerCase()));let rip=(cur.objcat==="Wszystkie"||cur.objcat==="Riposty")?RIPOSTES:[];if(v)rip=rip.filter(o=>(o.q+o.a).toLowerCase().includes(v.toLowerCase()));let html=list.map(o=>objDetails(o,!!v)).join("");if(rip.length)html+=`<div class="paper" style="margin-top:16px"><h3>Riposty – jedno zdanie na obiekcję (${rip.length})</h3><table><tr><th>Klient</th><th>Ty</th></tr>${rip.map(o=>`<tr><td>${hl(o.q)}</td><td>${hl(o.a)}</td></tr>`).join("")}</table></div>`;view.innerHTML=html||`<p class="empty">Brak obiekcji pasujących.</p>`;return;}
  if(cur.sec==="wiedza"){const c=cur.calc;
    ...
    else{setTitle("Umowa ESCO – playbook","Kiedy ESCO ma sens, co musi być w umowie, jak mówić do CFO i skarbnika.");view.innerHTML=`<div class="paper"><p>${hl(ESCO.intro)}</p><h3>Kiedy ESCO ma sens</h3>${ul(ESCO.when)}<h3>Co musi regulować umowa</h3>${checklist(ESCO.contract,"esco")}<h3>Rozmowa z finansowym</h3><p>${hl(ESCO.cfo)}</p><h3>Sektor publiczny</h3><p>${tagStatus(hl(ESCO.publicNote))}</p></div>`;}
    return;}
  if(cur.sec==="spotkanie"){const T=TOOLKIT;let html="";const t=cur.tool;
  ...
  if(cur.tab==="persona"){const ppl=(DECIDER_MAP[p.id]||[]).map(n=>DECIDERS.people.find(x=>x.t===n)).filter(Boolean);html=`<div class="paper"><h3>Z kim rozmawiasz</h3><p>${hl(p.who)}</p><h3>Jaki problem tej osoby rozwiązujemy</h3>${ul(p.pain)}<h3>Czego się boi</h3><p>${hl(p.fear)}</p><h3>Jak do niej mówić</h3><p>${hl(p.angle)}</p><div class="bar"><button class="ghost" data-mail="cold|${MAIL_MAP[p.id]}">Napisz pierwszego maila do tego segmentu</button><button class="ghost" data-mail="summary|${MAIL_MAP[p.id]}">Mail po spotkaniu</button></div></div>`+(ppl.length?`<h3 style="font-size:14px;color:var(--ink-2);margin:0 0 8px">Jak rozmawiać z konkretnymi osobami</h3>`+ppl.map(x=>`<details><summary>${hl(x.t)}</summary><div class="body"><p>${hl(x.how)}</p></div></details>`).join(""):"")+`<details style="margin-top:12px"><summary>Trzy typy rozmówców – techniczny, finansowy, decyzyjny</summary><div class="body">`+deciderTypes()+`</div></details>`;}
  else if(cur.tab==="diag")html=`<div class="paper"><h3>Najlepsze otwarcie</h3><p>${hl(p.opening)}</p><h3>Pytania diagnostyczne (${p.diag.length})</h3><ol>${p.diag.map(x=>`<li>${hl(x)}</li>`).join("")}</ol><h3>Pytania, po których klient sam nazywa stratę</h3><p style="font-family:'IBM Plex Sans';font-size:13.5px;color:var(--ink-2)">Zadaj, zamilknij, zanotuj odpowiedź. Nie odpowiadaj za klienta.</p>${ul(p.insight)}</div>`;
  else if(cur.tab==="args")html=`<div class="paper"><h3>Techniczne</h3>${ul(A.tech)}<h3>Ekonomiczne</h3>${ul(A.econ)}<h3>Dla zarządu / właściciela</h3>${ul(A.board)}<h3>Prawne i regulacyjne</h3>${ul(A.legal)}<h3>ESG, efektywność, dekarbonizacja</h3>${ul(A.esg)}</div>`+((p.id==="pec"||p.id==="operator")?`<h3 style="font-size:14px;color:var(--ink-2);margin:16px 0 8px">Ciepłownie – konflikt interesów i argumenty z weryfikacją</h3>`+pecChapter():"");
  else if(cur.tab==="obj")html=`<h3 style="font-size:14px;color:var(--ink-2);margin:0 0 8px">Specyficzne dla segmentu</h3>`+p.objections.map(o=>objDetails(o,!!q())).join("")+`<p class="note" style="margin-top:12px">Pozostałe obiekcje i riposty: Wiedza › Biblioteka obiekcji.</p>`;
...
function renderHits(){const v=q().toLowerCase();const hits=[];
  SEGMENTS.forEach(s=>{const fields={persona:[s.who,s.fear,s.angle,...s.pain],diag:[s.opening,...s.diag],insight:s.insight,args:[].concat(...Object.values(s.args)),obj:s.objections.map(o=>o.q+" "+o.a),path:[s.offerMoment,...s.path,...s.close],dialog:s.dialog.map(d=>d[1])};for(const t in fields){const m=fields[t].find(x=>x.toLowerCase().includes(v));if(m)hits.push({go:{sec:"klienci",seg:s.id,tab:t==="insight"?"diag":t},title:s.name+" › "+(t==="insight"?"Otwarcie i diagnoza":SEG_TABS.find(x=>x[0]===t)[1]),snip:m});}});
  LEGAL.forEach(l=>{const m=[l.act,l.what,l.use,l.trap,l.who].find(x=>x.toLowerCase().includes(v));if(m)hits.push({go:{sec:"wiedza",calc:"prawo"},title:"Prawo › "+l.act,snip:m});});
  PEC_CHAPTER.args.forEach(a=>{const m=[a.t,a.why,a.how].find(x=>x.toLowerCase().includes(v));if(m)hits.push({go:{sec:"klienci",seg:"pec",tab:"args"},title:"PEC › argumenty › "+a.t,snip:m});});
  DECIDERS.people.forEach(p=>{if((p.t+p.how).toLowerCase().includes(v)){const sid=Object.keys(DECIDER_MAP).find(k=>DECIDER_MAP[k].includes(p.t))||"urzad";hits.push({go:{sec:"klienci",seg:sid,tab:"persona"},title:"Kto decyduje › "+p.t,snip:p.how});}});
  OBJ_LIB.forEach(o=>{if((o.q+o.s+o.l).toLowerCase().includes(v))hits.push({go:{sec:"wiedza",calc:"obiekcje",objcat:"Wszystkie"},title:"Obiekcje › "+o.q,snip:o.s});});
  TOOL_ITEMS.forEach(([id])=>{const src=id==="fu"?TOOLKIT.followupSchema:id==="stages"?TOOLKIT.stages:TOOLKIT[id];const flat=JSON.stringify(src).toLowerCase();if(flat.includes(v))hits.push({go:{sec:"spotkanie",tool:id},title:"Spotkanie › "+TOOL_ITEMS.find(x=>x[0]===id)[1],snip:""});});
  RIPOSTES.forEach(o=>{if((o.q+o.a).toLowerCase().includes(v))hits.push({go:{sec:"wiedza",calc:"obiekcje",objcat:"Riposty"},title:"Riposty › "+o.q,snip:o.a});});
...
function render(){renderNav();renderSide();renderView();$root.querySelector(".hb-main").classList.toggle("wide",cur.sec==="spotkanie"&&cur.tool==="maile");}
...
$id("side").addEventListener("click",e=>{const b=e.target.closest("button");if(!b||b.dataset.mail)return;if(b.dataset.go){Object.assign(cur,JSON.parse(b.dataset.go));cur.hitsMode=false;render();return;}if(b.dataset.seg){cur.seg=b.dataset.seg;}if(b.dataset.tool){cur.tool=b.dataset.tool;}if(b.dataset.cat){cur.objcat=b.dataset.cat;}if(b.dataset.calc){cur.calc=b.dataset.calc;}if(b.dataset.acad){cur.acad=b.dataset.acad;}if(b.dataset.goseg){cur.sec="klienci";cur.seg=b.dataset.goseg;cur.tab="persona";}cur.hitsMode=false;render();});

function openMail(spec){const [type,rec]=spec.split("|");MAIL.want={type,rec};cur.sec="spotkanie";cur.tool="maile";cur.hitsMode=false;render();}
...
$id("view").addEventListener("click",e=>{
  ...
  if(b.dataset.go){Object.assign(cur,JSON.parse(b.dataset.go));cur.hitsMode=false;render();return;}
...
if(!SECTIONS.some(s=>s[0]===cur.sec))cur.sec="start";
render();
